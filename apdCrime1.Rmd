...
Can we color the beats or zones by total number of homicides?

```{r}
homocidesByBeat <- apdCrimeDataClean %>% 
  filter(UC2_Literal=="HOMICIDE") %>% 
  group_by(beat) %>% 
  summarise(freq = n()) %>% 
  ungroup()
# heatCols <- heat.colors(nrow(homocidesByBeat))[cut(sort(homocidesByBeat$freq),nrow(homocidesByBeat))]
pal <- colorNumeric(
  palette = "YlGnBu",
  domain = homocidesByBeat$freq
)
apdCrimeDataClean %>% 
  # filter(UC2_Literal=="HOMICIDE") %>% 
  leaflet() %>% 
  ...
  # addPolygons(data=beats, stroke=FALSE, fillOpacity = 0.5, smoothFactor=0.5,
              # fillColor=heatCols) %>% 
  addPolygons(data=beats, stroke=FALSE, fillOpacity = 0.5, smoothFactor = 0.5,
              color = ~pal(homocidesByBeat$freq)) %>% 
  addLegend("bottomright", pal = pal, values = homocidesByBeat$freq,
            title = "Legend",
            opacity = 1)
```
